...
class CacheEnv(gym.Env):
  ...
  def __init__(self):
    ...
    self.state = np.zeros(shape=(self.edge_n, (self.user_n * self.each_user_task_n+self.each_edge_cache_n)))
  ...
  def observation_space(self):
    return [spaces.Box(low=0, high=self.task_n, shape=(self.user_n+self.each_edge_cache_n, 1)) for i in range(self.edge_n)]
  ...
  def action_space(self):
    return [spaces.Discrete(np.power(2, self.user_n)*np.power(self.task_n, self.each_edge_cache_n)) for i in range(self.edge_n)]
  ...
  def action_wrapper_32000(self, acs):
    acs = acs[0]
    ac_str = []
    serve_ac = np.zeros(shape=(len(acs), self.user_n))
    cache_ac = np.zeros(shape=(len(acs), self.each_edge_cache_n))
    for i in range(len(acs)):
      action = list(acs[i]).index(1)
      serve_num = action // np.power(self.task_n, self.each_edge_cache_n)
      cache_num = action % np.power(self.task_n, self.each_edge_cache_n)
      cache_ac[i][0] = cache_num // np.power(self.task_n, 2)
      temp = cache_num % np.power(self.task_n, 2)
      cache_ac[i][1] = temp // self.task_n
      cache_ac[i][2] = temp % self.task_n
  ...
  def step(self, action):
    self.step_num += 1
    serve_action, cache_action = self.action_wrapper_32000(action)
    ...
    for i in range(self.user_n):
      for j in range(self.edge_n):
        ...
        if self.users_tasks[i] in self.edge_caching_task[j]:
          ...
          if ac[i] == 1:
            serve_success[j][i] = 1
      if 1 not in cache_success[:, i]:
        cache_user_fail[i] = 1

    #reward_cache_user_fail = 10*sum(cache_user_fail)
    reward_success = 100*(np.sum(serve_success, axis=1)/self.max_cache_num(self.users_tasks))
    #reward = reward_success - reward_cache_user_fail
    ...
    if self.step_num == 1 or self.step_num == 50 or self.step_num == 99:
      ...
      #print("user_fail:", -reward_cache_user_fail)
      print("reward_success:", reward_success)
      print("reward: ", reward, '\n\n\n')


    self.edge_caching_task = cache_action
    for i in range(self.edge_n):
      self.state[i] = np.append(self.users_tasks, self.edge_caching_task[i])
